...
class MyApp(App):
    async def on_mount(self):
        ...
        if not is_dependency_installed("some_dependency"):
            log("Dependency not installed, attempting to install...")
            await self.install_dependency("some_dependency")

    async def install_dependency(self, dependency_name):
        # Attempt to install the dependency
        try:
            await install_and_push_modal(self, dependency_name)
            log(f"Successfully installed {dependency_name}")
        except Exception as e:
            log(f"Failed to install {dependency_name}: {e}")
